...
namespace ERP.SE
{
    public partial class FormSEOrder : Form
    {
        ...
        private void toolSave_Click(object sender, EventArgs e)
        {
            ...
            DataSet ds = db.GetDataSet("select (Quantity-LossQuantity) from STStock where StoreCode='" + cbxStoreCode.SelectedValue.ToString() + "' and InvenCode='" + cbxInvenCode.SelectedValue.ToString() + "'", "STStock");
            if (ds.Tables[0].Rows.Count > 0)
            {
                if (Convert.ToInt32(txtQuantity.Text.Trim()) > Convert.ToInt32(ds.Tables[0].Rows[0][0]))
                {
                    MessageBox.Show("销售数量不能大于库存数量", "软件提示");
                    txtQuantity.Focus();
                    return;
                }
            }
            ...
            {
                MessageBox.Show("当前仓库中没有该商品，请重新选择", "软件提示");
                txtQuantity.Focus();
                return;
            }
            //添加
            if (toolStrip1.Tag.ToString() == "ADD")
            {
                try
                {
                    ...
                    if (db.ExecDataBySql(strCode) > 0)
                    {
                        ...
                        db.ExecDataBySql("update STStock set Quantity=(Quantity-" + Convert.ToInt32(txtQuantity.Text.Trim()) + ") where StoreCode='" + cbxStoreCode.SelectedValue.ToString() + "' and InvenCode='" + cbxInvenCode.SelectedValue.ToString() + "'");//更新库存
