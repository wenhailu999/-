...
namespace ERP.BS
{
    public partial class FormBSInvenTypeInput : Form
    {
        ...
        FormBSInvenType formInvenType = null;
        ...
        {
            formInvenType = (FormBSInvenType)this.Owner;
        ...
        private void btnOk_Click(object sender, EventArgs e)
        {
            ...
            if (this.Tag.ToString() == "Add") //添加操作
            {
                ...
                try
                {
                    ...
                    {
                        sdr.Close();
                        strCode = "INSERT INTO BSInvenType(InvenTypeCode,InvenTypeName) VALUES('" + txtTypeCode.Text.Trim() + "','" + txtTypeName.Text.Trim() + "')";
                        
                        if (db.ExecDataBySql(strCode) > 0)
                        {
                            MessageBox.Show("保存成功！", "软件提示");
                            commUse = new CommonUse();
                            commUse.BuildTree(formInvenType.tvInvenType, formInvenType.imageList1, "存货分类", "BSInvenType", "InvenTypeCode", "InvenTypeName");
            ...
            else //修改
            {
                ...
                try
                {
                    strCode = "UPDATE BSInvenType SET InvenTypeCode = '" + txtTypeCode.Text.Trim() + "',InvenTypeName = '" + txtTypeName.Text.Trim() + "' WHERE InvenTypeCode = '" + formInvenType.tvInvenType.SelectedNode.Tag.ToString() + "'";
                    if (db.ExecDataBySql(strCode) > 0)
                    {
                        MessageBox.Show("保存成功！", "软件提示");
                        commUse = new CommonUse();
