     ...
     public class Chart
    {
         ...
         public Image CreatePieChart(string title, string subTitle, int width, int height, DataTable dt, int ColumnIndex)
         {
             ...
             for (int i = 0; i < dt.Rows.Count; i++)
             {
                 ...
                 g.FillPie(new SolidBrush(GetChartItemColor(i)), 100, 65, PIE_DIAMETER, PIE_DIAMETER, totalAngle, curAngle);
             ...
             for (int i = 0; i < dt.Rows.Count; i++)
             {
                 ...
                 g.DrawString(dt.Rows[i][1].ToString() + "—" + dt.Rows[i][2].ToString() + "(" + percent.ToString("0") + "%)", new Font("Tahoma", 10), Brushes.Black, textOrigin);
                 boxOrigin.Y += 15;
                 textOrigin.Y += 15;
             }
             //释放资源，同时返回绘制的饼形图
